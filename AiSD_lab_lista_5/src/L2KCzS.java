...
public class L2KCzS<E> implements Serializable {
    ...
    public boolean add(E value) {
        if (value.getClass() == new Pracownik(1,"a","b","1","s",1,1).getClass()) {
            if (this.isEmpty()) {
                ...
                Element elem = sentinel.getNext();
                Pracownik nowy  = (Pracownik) value;
                boolean added = false;
                int counter = 0;
                while (elem != sentinel && counter<=this.size()){
                    Pracownik current = (Pracownik) elem.getValue();
                    if (nowy.getPesel()>current.getPesel()){
    ...
    public int indexOf(E value) {
        if(value.getClass() == new Pracownik(1,"a","b","1","s",1,1).getClass()){
            ...
            int counter=0;
            Pracownik current = (Pracownik) elem.getValue();
            Pracownik pracownik = (Pracownik) value;
            while(elem!=sentinel && current.getPesel() == pracownik.getPesel()){
                counter++;
                elem=elem.getNext();
                current = (Pracownik) elem.getValue();
            }
            if(elem==sentinel)
                return -1;
            return counter;
    ...
    public TWCIterator iterator() {
        return new TWCIterator();}
    private class TWCIterator implements Iterator<E>, Serializable{
    ...
    public TWCListIterator listIterator() {
